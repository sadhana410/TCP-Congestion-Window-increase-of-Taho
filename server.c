...
struct Packet {
    int seq;
    char data[10]; // Adjust data size as needed
};
int count = 1;
struct Packet ack;
int prev=0;
...
int main(int argc, char **argv) {
    ...
    /* for (int i = 0; i < (size+1); i++) {
        char intnum[30];
        snprintf(intnum, sizeof(intnum), "%d", i);
        char str[] = "Packet - ";
        strcat(str, intnum);
        strcpy(packets[i].data, str);
        packets[i].seq = i;
    } */

    processBinaryData("input_data.txt", packets, size);
